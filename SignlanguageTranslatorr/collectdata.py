...
directory='Image/'
while True:
    _,frame=cap.read() #captue and process frames
    count = { # counting existing images
    ...
    cv2.imshow("data",frame) #display frames
    cv2.imshow("ROI",frame[40:400,0:300])
    frame=frame[40:400,0:300]
    interrupt = cv2.waitKey(10) #capture key pree
    if interrupt & 0xFF == ord('a'): #save images based on key process
...
cap.release() #release resources and close window
